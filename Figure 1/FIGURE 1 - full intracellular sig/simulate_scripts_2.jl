...
# To fill up the figures: function returnTuringParams(model, model_params; maxiters = 1e3,alg=Rodas5(),abstol=1e-8, reltol=1e-5, tspan=1e4,ensemblealg=EnsembleThreads(), sample_mode = "sparse") 

function returnTuringParams(model, params; maxiters = 1e3,alg=Rodas5(),abstol=1e-8, reltol=1e-5, tspan=1e4,ensemblealg=EnsembleThreads()) 

#function returnTuringParams(model, params; maxiters = 1e3,alg=Rodas5(),abstol=1e-8, reltol=1e-8, tspan=1e4,ensemblealg=EnsembleThreads())   #Daniel changed reltol from 1e-6 to 1e-8, Originally: abstol=1e-8, reltol=1e-6  #THIS LINE IS THE NORMALLY USED
